...
<!-- Transformační šablona náhledu pro pokladní doklady ve tvaru HTML. Doklad se zobrazuje jako náhled dokladu před tiskem. - SK lokalizácia
Autor: Marek Vykydal
 -->
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>Pokladničný doklad</title>
				<style><![CDATA[
	...
	<!-- -->
	<xsl:template match="SeznamPokDokl">
		<xsl:apply-templates select="PokDokl">
			<xsl:with-param name="Pocet" select="count(PokDokl)"/>
		</xsl:apply-templates>
	</xsl:template>
						...
						<tbody>
							<tr>
								<td class="velikost1 tucne podtrzeni_N3" align="left" height="32">
									<xsl:choose>		<!-- test, zda se jedná o příjmový nebo výdajový pokladní doklad  -->
										<xsl:when test="Vydej=0">Príjmový pokladničný doklad</xsl:when>
										<xsl:when test="Vydej=1">Výdajový pokladničný doklad</xsl:when>
										<xsl:otherwise>Pokladničný doklad</xsl:otherwise>
									</xsl:choose>
								</td>
							</tr>
						</tbody>
			...
			<tr>
              ...
              <tr>
                ...
                <td class="velikost4 zarovnani_D">
                  &#160;&#160;<xsl:value-of select="Adresa/DICSK" />
                </td>
              </tr>
						</tbody>
					</table>
					...
					<table class="podtrzeni_N podtrzeni_D podtrzeni_L podtrzeni_P" bordercolor="black" width="100%" cellspacing="0" cellpadding="5" border="0">
						<tbody>
							<tr>
								...
								<td class="velikost4 zarovnani_N podtrzeni_P" width="25%">Podpis pokladníka:</td>
								<td class="velikost4 zarovnani_N">Podpis príjemca:</td>
							</tr>
							...
							<tbody>
									<xsl:choose>
										<xsl:when test="string-length(SeznamRozuctPolozek/RozuctPolozka/Pohyb)>0">	<!-- daňová evidence  -->
											...
											<tr>
												<td class="velikost5 tucne podtrzeni_D " height="21" align="left" width="35%">Popis - rozúčtovania</td>
			...
			<!-- cenový přehled dokladu -->
				...
				<td>
					<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
						<tbody>
						<tr>
							<td class="velikost2 tucne" height="25" align="right" width="43%">Suma s DPH:&#160;&#160;</td>
