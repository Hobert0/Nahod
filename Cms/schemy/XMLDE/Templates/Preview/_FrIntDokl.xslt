...
<!-- Transformační šablona náhledu pro interní doklady ve tvaru HTML. Doklad se zobrazuje jako náhled dokladu před tiskem. SK lokalizácia
Autor: Marek Vykydal
 -->
	...
	<xsl:template match="SeznamIntDokl">
		<xsl:apply-templates select="IntDokl">
			<xsl:with-param name="Pocet" select="count(IntDokl)"/>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:if test="name(MojeFirma)">			<!-- není prozatím podporován export elementu MojeFirma, proto je zde podmínka pro pozdější budoucí použití -->
			...
			<tr>
				...
				<td class="zarovnani_N" width="48.5%">
					<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
						<tbody>
							<tr>
								<td class="velikost1 tucne podtrzeni_N3" align="left" height="32">Interný doklad</td>
							</tr>
						</tbody>
					</table>
				</td>
				...
				<td class="zarovnani_N" rowspan="2">
					<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
						<tbody>
							...
							<tr>	<td class="velikost4 tucne podtrzeni_D" colspan="3">Adresa</td>
							</tr>
			...
			<tr>	
				<td colspan="3">
					...
					<xsl:when test="count(RozuctPolozka)>0">		<!-- rozúčtovací položky  - vždy pouze v domácí měně  -->
						<table bordercolor="green" cellspacing="0" cellpadding="0" width="100%" border="0" >
							<tbody>
									<xsl:choose>
										<xsl:when test="string-length(RozuctPolozka/Pohyb)>0">	<!-- daňová evidence  -->
											...
											<xsl:for-each select="RozuctPolozka">
												<tr>
													<td class="velikost5 podtrzeni_NT" height="23" align="left"><xsl:value-of select="Popis"/></td>
			...
			<tr>
				<xsl:choose>
					<xsl:when test="count(SeznamNormPolozek/NormPolozka)>0 or count(RozuctPolozka)>0">
