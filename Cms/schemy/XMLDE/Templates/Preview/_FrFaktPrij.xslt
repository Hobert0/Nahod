...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<!-- Transformační šablona náhledu pro faktury přijaté ve tvaru HTML. Doklad se zobrazuje jako náhled dokladu před tiskem.
Autor: Marek Vykydal
 -->
	...
	<!-- -->
	<xsl:template match="SeznamFaktPrij | SeznamFaktPrij_DPP">
		<xsl:apply-templates select="FaktPrij | FaktPrij_DPP">
			<xsl:with-param name="Pocet" select="count(FaktPrij | FaktPrij_DPP)"/>
		</xsl:apply-templates>
	</xsl:template>
	...
	<xsl:template name="_datum_">
		...
		<xsl:variable name="datum" select="concat($den,'.',$mesic,'.',$rok)"/>
	...
	<xsl:template match="Slozeni/SubPolozka/Polozka">
		...
		<tr>
			...
			<td class="velikost5 " align="right">&#160;
						<xsl:if test="($TypKarty != 'sada') and ($TypKarty != 'komplet')">
					<xsl:choose>
		...
		<xsl:if test="count(SeznamVC/VyrobniCislo)>0">
			<xsl:for-each select="SeznamVC/VyrobniCislo">
				<tr>
					...
					<td class="velikost6 kurziva" colspan="7">
						<xsl:value-of select="VyrobniCis"/>
					</td>
				</tr>
			</xsl:for-each>
		</xsl:if>
	...
	<!-- Faktura  přijatá  -->
	<xsl:template match="FaktPrij | FaktPrij_DPP">
		...
		<table bordercolor="blue" width="100%" cellspacing="0" cellpadding="0" border="0">
			<tbody>
				...
				<tr>
					<td colspan="3">
						<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
							<tbody>
								<tr>
									<td height="20" class="velikost2 tucne" colspan="8">
										<xsl:value-of select="MojeFirma/Nazev"/>&#160;</td>
								</tr>
								<tr>
									<td class="velikost3 tucne zarovnani_N" width="28%">
										...
										<xsl:value-of select="MojeFirma/ObchAdresa/Misto"/>
										<br/>
										<xsl:value-of select="MojeFirma/ObchAdresa/Stat"/>&#160;
								</td>
									<td class="velikost3 tucne zarovnani_N" align="right" width="5%">IČO:<br/>IČ DPH:<br/>DIČ: </td>
									<td class="velikost3 tucne zarovnani_N" width="13%">
										&#160;&#160;<xsl:value-of select="MojeFirma/ICO"/>
										<br/>
										&#160;&#160;<xsl:value-of select="MojeFirma/DIC"/>
										<br/>
									...
									<td class="velikost7 zarovnani_N" width="14%">
										&#160;&#160;<xsl:value-of select="MojeFirma/Mobil/Pred"/>
										<xsl:if test="string-length(MojeFirma/Mobil/Pred)>0 ">-</xsl:if>
										<xsl:value-of select="MojeFirma/Mobil/Cislo"/>
										<br/>
										&#160;&#160;<xsl:value-of select="MojeFirma/WWW"/>
										<br/>
									...
									<td class="velikost7 zarovnani_N" width="30%">
										&#160;&#160;<xsl:value-of select="MojeFirma/Tel/Pred"/>
										<xsl:if test="string-length(MojeFirma/Tel/Pred)>0 ">-</xsl:if>
										<xsl:value-of select="MojeFirma/Tel/Cislo"/>
										<xsl:if test="string-length(MojeFirma/Tel/Klap)>0 ">/</xsl:if>
										<xsl:value-of select="MojeFirma/Tel/Klap"/>
										<br/>

										&#160;&#160;<xsl:value-of select="MojeFirma/Fax/Pred"/>
										<xsl:if test="string-length(MojeFirma/Fax/Pred)>0 ">-</xsl:if>
								...
								<tr>
									<td class="velikost1 tucne podtrzeni_N3" align="left" height="32">
										<xsl:choose>
											...
											<xsl:when test="Druh='L' or Druh='Z' ">Zálohová faktúra prijatá</xsl:when>
											<xsl:when test="Druh='F' or Druh='P' ">Proforma faktura prijatá</xsl:when>
											<xsl:when test="Druh='N' and Dobropis='1' ">Faktúra prijatá - dobropis</xsl:when>
											<xsl:otherwise>Faktúra prijatá</xsl:otherwise>
										</xsl:choose>
									</td>
								</tr>
							</tbody>
					...
					<td class="zarovnani_N" rowspan="2">
						<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
							<tbody>
								...
								<tr>
									<td class="velikost4 tucne">Prijatý doklad:</td>
									<td class="velikost3 tucne pozadi1" align="center" colspan="2">
										<xsl:value-of select="PrijatDokl"/>
									</td>
								</tr>
								<tr>
								...
								<tr>
									<td class="velikost4 tucne podtrzeni_D" colspan="3">Dodávateľ</td>
								</tr>
								<tr>
								...
								<tr>
									<td class="velikost3 tucne" colspan="3">
										<xsl:if test="DodOdb/Nazev != DodOdb/ObchNazev">
											<xsl:value-of select="DodOdb/Nazev"/>
										</xsl:if>&#160;<br/>
										<xsl:value-of select="DodOdb/ObchAdresa/Ulice"/>&#160;<br/>
										<xsl:value-of select="DodOdb/ObchAdresa/PSC"/>&#160;&#160;
									<xsl:value-of select="DodOdb/ObchAdresa/Misto"/>
										<br/>
										<xsl:value-of select="DodOdb/ObchAdresa/Stat"/>&#160;
								</td>
								</tr>
								<tr>
									<td class="podtrzeni_D" height="20" colspan="3">&#160;</td>
								</tr>
							</tbody>
						</table>
					</td>
							...
							<tbody>
								<tr>
									...
									<td class="velikost4" colspan="2">
										<xsl:if test="Druh!='D' ">
											<xsl:value-of select="Uhrada"/>&#160;</xsl:if>
									</td>
									<td class="velikost9 pismo1 tucne" align="center">
										<!-- <xsl:if test="Dobropis=1 ">ţ</xsl:if> -->
									</td>
									<td class="velikost4 tucne" colspan="4">
										<!-- xsl:if test="Dobropis=1 ">Opravný daňový doklad</xsl:if> -->
									</td>
								</tr>
								<tr>
									...
									<td class="velikost4" colspan="7">
										<xsl:if test="Druh!='D' ">
											<xsl:value-of select="ZpDopravy"/>&#160;</xsl:if>
									</td>
								</tr>
								<tr>
				...
				<tr>
							...
							<tbody>
								<tr>
									...
									<td class="velikost4" width="16%">
										<xsl:choose>
											<xsl:when test="Druh!='D' ">&#160;&#160;<xsl:value-of select="KonstSym"/>
											</xsl:when>
								...
								<tr>
									...
									<td/>
									<td class="velikost4 tucne pozadi1" colspan="2">&#160;&#160;<xsl:value-of select="VarSymbol"/>
									</td>
								</tr>
								<tr>
									...
									<td class="velikost4" align="center">
										<xsl:choose>
											...
											<xsl:when test="(Druh='L' or Druh='F') and (Vyrizeno!='') ">
												<xsl:call-template name="_datum_">
													<xsl:with-param name="_datum">
									...
									<td class="velikost4" colspan="2">
										<xsl:choose>
											<xsl:when test="Druh!='D' ">&#160;&#160;<xsl:value-of select="SpecSymbol"/>
											</xsl:when>
											<xsl:otherwise>-</xsl:otherwise>
										</xsl:choose>
									</td>
								...
								<tr>
									<xsl:choose>
										<xsl:when test="Druh!='D' ">
											<td class="velikost3 tucne pozadi1" colspan="3">Platba na:</td>
											<td/>
											<td class="velikost3 tucne" align="right" colspan="4">
												<xsl:value-of select="DodOdb/Banka"/>
											</td>
										</xsl:when>
										<xsl:otherwise>
											<td/>
									...
									<xsl:choose>
										<xsl:when test="Druh!='D' ">
											<td class="velikost2 tucne podtrzeni_N3 podtrzeni_D3 podtrzeni_P3 podtrzeni_L3" align="center" colspan="5">
												<xsl:value-of select="DodOdb/Ucet"/>
												<xsl:if test="string-length(DodOdb/Ucet)=0">&#160;</xsl:if>
											</td>
											<td/>
											<td class="velikost2 tucne podtrzeni_N3 podtrzeni_D3 podtrzeni_P3 podtrzeni_L3" align="center" colspan="2">
												<xsl:value-of select="DodOdb/KodBanky"/>
												<xsl:if test="string-length(DodOdb/KodBanky)=0">&#160;</xsl:if>
											</td>
										</xsl:when>
										<xsl:otherwise>
											<td/>
					...
					<td class="zarovnani_N">
						<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
							<tbody>
								<tr>
									<td class="velikost4 tucne" rowspan="3">
										<xsl:if test="string-length(KonecPrij/Nazev)>0">Konečný príjemca</xsl:if>&#160;</td>
									<td class="velikost4 zarovnani_D" align="right" height="17" width="10%">IČO:</td>
									<td class="velikost4 zarovnani_D" width="30%">&#160;&#160;<xsl:value-of select="DodOdb/ICO"/>
									</td>
								</tr>
								<tr>
									<td class="velikost4 zarovnani_D" align="right">IČ DPH:</td>
									<td class="velikost4 zarovnani_D">&#160;&#160;<xsl:value-of select="DodOdb/DIC"/>
									</td>
								</tr>
								<tr>
									<td class="velikost4 zarovnani_D" align="right">DIČ: </td>
									<td class="velikost4 zarovnani_D">&#160;&#160;<xsl:value-of select="DodOdb/DICSK"/>
									</td>
								</tr>
							</tbody>
						</table>
					...
					<td class="zarovnani_D">
						<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
							<tbody>
								<tr>
									<td class="velikost4">
										<xsl:if test="string-length(KonecPrij/Nazev)>0">
											<xsl:value-of select="KonecPrij/Nazev"/>&#160;<br/>
											<xsl:value-of select="KonecPrij/Adresa/Ulice"/>&#160;<br/>
											<xsl:value-of select="KonecPrij/Adresa/PSC"/>&#160;&#160;<xsl:value-of select="KonecPrij/Adresa/Misto"/>
											<br/>
						...
						<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
							<tbody>
									...
									<tr>
										...
										<td class="velikost5 tucne podtrzeni_D" align="right">
											<xsl:choose>
												...
												<xsl:otherwise>Cena celkem</xsl:otherwise>
											</xsl:choose>
										</td>
								...
								<xsl:for-each select="SeznamPolozek/Polozka">
									...
									<xsl:if test="string-length(Poznamka)>0">
										<tr>
											<td class="velikost7 kurziva zarovnani_N" height="15" colspan="9">
												<xsl:value-of select="Poznamka"/>
											</td>
										</tr>
									</xsl:if>
									<xsl:if test="count(SklPolozka/SeznamVC/VyrobniCislo)>0">
										<xsl:for-each select="SklPolozka/SeznamVC/VyrobniCislo">
											<tr>
												...
												<td class="velikost6 kurziva" colspan="8">
													<xsl:value-of select="VyrobniCis"/>
												</td>
											</tr>
										</xsl:for-each>
										...
										<xsl:choose>
											<xsl:when test="string-length(Valuty/Mena/Kod)>0">
												<xsl:choose>
													...
													<xsl:otherwise>
														<xsl:value-of select="format-number(Valuty/Celkem, '#,##0.00')"/>&#160;&#160;</xsl:otherwise>
												</xsl:choose>
											</xsl:when>
											<xsl:otherwise>
												<xsl:choose>
													...
													<xsl:otherwise>
														<xsl:value-of select="format-number(Celkem, '#,##0.00')"/>&#160;&#160;</xsl:otherwise>
												</xsl:choose>
											</xsl:otherwise>
										</xsl:choose>
									</td>
									...
									<tr>
										...
										<td class="velikost2 tucne" align="right">
											<xsl:choose>
												...
												<xsl:otherwise>
													<xsl:value-of select="format-number((SumZaloha)*(-1), '#,##0.00')"/>&#160;&#160;</xsl:otherwise>
											</xsl:choose>
										</td>
										<td/>
									</tr>
									...
									<tr>
										...
										<td class="velikost2 tucne" align="right">
											<xsl:choose>
												<xsl:when test="string-length(Valuty/Mena/Kod)>0">
													<xsl:choose>
														<!-- u dobropisů se obrací znaménko - viz totéž co v tisku -->
														<xsl:when test="Dobropis = 1">
															<xsl:value-of select="format-number((ValutyProp)*(-1), '#,##0.00')"/>&#160;&#160;</xsl:when>
														<xsl:otherwise>
															<xsl:value-of select="format-number(ValutyProp, '#,##0.00')"/>&#160;&#160;</xsl:otherwise>
													</xsl:choose>
												</xsl:when>
												<xsl:otherwise>
													<xsl:choose>
														<!-- u dobropisů se obrací znaménko - viz totéž co v tisku -->
														<xsl:when test="Dobropis = 1">
															<xsl:value-of select="format-number((Proplatit)*(-1), '#,##0.00')"/>&#160;&#160;</xsl:when>
														<xsl:otherwise>
															<xsl:value-of select="format-number(Proplatit, '#,##0.00')"/>&#160;&#160;</xsl:otherwise>
													</xsl:choose>
												</xsl:otherwise>
											</xsl:choose>
										</td>
				...
				<tr>
					<td colspan="10">&#160;<div style="page-break-after: always"/>
					</td>
				</tr>
