...
<!-- Transformační šablona náhledu pro skladové doklady ve tvaru HTML. Doklad se zobrazuje jako náhled dokladu před tiskem. - SK lokalizácia
Autor: Marek Vykydal
 -->
	...
	<!-- -->
	<xsl:template match="SeznamPrijemka|SeznamVydejka|SeznamDLPrij|SeznamDLVyd|SeznamProdejka|SeznamPrevodka">
		<xsl:apply-templates select="Prijemka|Vydejka|DLPrij|DLVyd|Prodejka|Prevodka">
			<xsl:with-param name="Pocet" select="count(Prijemka|Vydejka|DLPrij|DLVyd|Prodejka|Prevodka)"/>
		</xsl:apply-templates>
	</xsl:template>
		...
		<xsl:variable name="TypKarty">
			...
			<xsl:value-of select="KmKarta/TypKarty" />
				...
				<tr>
					...
					<td class="velikost5 " align="right">&#160;
						<xsl:if test="((($Druh = 'DLVyd') or ($Druh = 'Prodejka')) and ($TypKarty != 'sada') and ($TypKartyNadr != 'komplet'))
						or ((($Druh = 'DLPrij') or ($Druh = 'Prijemka') or ($Druh = 'Vydejka') or ($Druh = 'Prevodka')) and ($TypKarty != 'sada') and ($TypKarty != 'komplet'))">
							<xsl:choose>
...
<!-- Skladový doklad -->
	<xsl:template match="Prijemka|Vydejka|DLPrij|DLVyd|Prodejka|Prevodka">
							...
							<tr>
								<td class="velikost1 tucne podtrzeni_N3" align="left" height="32">
									<xsl:choose>
										...
										<xsl:when test="$Druh = 'Prijemka' ">
											Skladová príjemka
										</xsl:when>
										<xsl:when test="$Druh = 'Vydejka' ">
											Skladová výdajka
										</xsl:when>
										<xsl:when test="$Druh = 'DLPrij'">
											Dodací list prijatý
										</xsl:when>
										<xsl:when test="$Druh = 'DLVyd' ">
											Dodací list vystavený
										</xsl:when>
										<xsl:when test="$Druh = 'Prodejka' ">
											Skladová predajka
										</xsl:when>
										<xsl:when test="$Druh = 'Prevodka' ">
											Skladová prevodka
										</xsl:when>
										<xsl:otherwise>
											Skladový doklad<xsl:value-of select="$Druh"/>
										</xsl:otherwise>
									</xsl:choose>
								</td>
							</tr>
						...
						<tbody>
							...
							<tr>
								<td class="velikost4 tucne">
									...
									<xsl:choose>
										<xsl:when test="$Druh = 'DLVyd' ">
											<td class="velikost3 tucne pozadi1" align="center" colspan="2"><xsl:value-of select="CObjednavk"/></td>													</xsl:when>
										<xsl:when test="$Druh = 'Prodejka' ">
											<td class="velikost3 tucne pozadi1" align="center" colspan="2"><xsl:value-of select="CObjednavk"/></td>													</xsl:when>
										<xsl:when test="$Druh = 'DLPrij' ">
											<td class="velikost3 tucne pozadi1" align="center" colspan="2"><xsl:value-of select="PrimDoklad"/></td>													</xsl:when>
										<xsl:otherwise>
					...
					<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
						<tbody>
							<tr>
								<td class="velikost4 tucne">	
									<xsl:choose>
										<xsl:when test="$Druh = 'Prodejka' ">Platba:</xsl:when>
										<xsl:when test="$Druh = 'Prevodka' ">Prevod na sklad:</xsl:when>
										<xsl:otherwise>&#160;</xsl:otherwise>
									</xsl:choose>
								</td>
								<td/>
								<td class="velikost4" colspan="2">
									<xsl:choose>
										<xsl:when test="$Druh = 'Prodejka' ">v hotovosti</xsl:when>
										<xsl:when test="$Druh = 'Prevodka' ">
											<xsl:choose>
												<xsl:when test="string-length(SkladProPr/Nazev)>0">
													<xsl:value-of select="SkladProPr/Nazev"/>
												</xsl:when>
												<xsl:when test="string-length(SkladProPr/KodSkladu)>0">
													<xsl:value-of select="SkladProPr/KodSkladu"/>
												</xsl:when>
											</xsl:choose>
										</xsl:when>
										<xsl:otherwise>&#160;</xsl:otherwise>
							...
							<tr>
								...
								<td/>
								<td class="velikost4" colspan="2">&#160;</td>
							</tr>
				...
				<td class="zarovnani_N" rowspan="2">
					<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
						<tbody>
								...
								<td class="velikost4" align="center" width="18%">
									<xsl:if test="string-length(Datum)>0">
										<xsl:call-template name="_datum_">
											<xsl:with-param name="_datum"><xsl:value-of select="Datum"/></xsl:with-param>
										</xsl:call-template>
									</xsl:if>
								</td>
								<td width="60%"/>
							...
							<xsl:if test="string-length(DatSkPoh)>0">
								<tr>
									<td class="velikost4" height="17" align="right">skl. pohybu:</td>
									<td/>
									<td class="velikost4" align="center">
										<xsl:call-template name="_datum_">
											<xsl:with-param name="_datum">
												<xsl:value-of select="DatSkPoh" />
											</xsl:with-param>
										</xsl:call-template>
									</td>
									<td/>
							...
							<xsl:if test="$Druh = 'Prodejka' ">
								<tr>	<td height="20" colspan="4"/>	</tr>
								<tr>	<td class="velikost1 tucne" height="17" colspan="4">Uhradené v hotovosti</td></tr>
							</xsl:if>
					...
					<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
						<tbody>
							...
							<!-- položky dokladu -->
								...
								<tr>
									<td class="velikost5 podtrzeni_NT" align="left" height="23" colspan="3"><xsl:value-of select="Nazev"/>&#160;</td>
									...
									<td class="velikost5 podtrzeni_NT" align="right">&#160;
										<xsl:if test="((($Druh = 'DLVyd') or ($Druh = 'Prodejka')) and ($TypKarty != 'sada'))
										or ((($Druh = 'DLPrij') or ($Druh = 'Prijemka') or ($Druh = 'Vydejka') or ($Druh = 'Prevodka')) and ($TypKarty != 'sada') and ($TypKarty != 'komplet'))">
											<xsl:choose>
												<xsl:when test="string-length($Mena)>0 and string-length(Valuty)>0">
													<xsl:value-of select="format-number(Valuty, '#,##0.00')" />
