...
<!-- Transformační šablona náhledu pro poptávky vydané ve tvaru HTML. Doklad se zobrazuje jako náhled dokladu před tiskem - sk lokalizácia.
Autor: Marek Vykydal
 -->
	...
	<xsl:template match="SeznamPoptVyd">
		<xsl:apply-templates select="PoptVyd">
			<xsl:with-param name="Pocet" select="count(PoptVyd)"/>
		</xsl:apply-templates>
	</xsl:template>
