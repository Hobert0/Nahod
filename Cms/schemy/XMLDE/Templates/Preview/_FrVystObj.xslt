...
<!-- Transformační šablona náhledu pro objednávky vydané ve tvaru HTML. Doklad se zobrazuje jako náhled dokladu před tiskem. - sk lokalizácia
Autor: Marek Vykydal
 -->
	...
	<xsl:template match="/">
		<html>
			<head>
				<title>Objednávka vystavená</title>
				<style><![CDATA[
	...
	<xsl:template match="SeznamObjVyd">
		<xsl:apply-templates select="ObjVyd">
			<xsl:with-param name="Pocet" select="count(ObjVyd)"/>
		</xsl:apply-templates>
	</xsl:template>
				...
				<td class="zarovnani_N" rowspan="2">
					<table bordercolor="black" width="100%" cellspacing="0" cellpadding="0" border="0">
						<tbody>
							...
							<tr>	<td class="velikost4 tucne podtrzeni_D" colspan="3">Dodavatel</td></tr>
