...
<!-- Transformační šablona náhledu pro poptávky přijaté ve tvaru HTML. Doklad se zobrazuje jako náhled dokladu před tiskem. - sk lokalizácia
Autor: Marek Vykydal
 -->
	...
	<!-- -->
	<xsl:template match="SeznamPoptPrij">
		<xsl:apply-templates select="PoptPrij">
			<xsl:with-param name="Pocet" select="count(PoptPrij)"/>
		</xsl:apply-templates>
	</xsl:template>
